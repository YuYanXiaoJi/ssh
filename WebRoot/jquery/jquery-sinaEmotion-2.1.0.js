...
(function($) {
		...
		$('#sinaEmotion').bind({
			...
			click: function(event) {
				...
				if (this.tagName !== 'INPUT' && this.tagName !== 'TEXTAREA') {
					...
					var imgText = '<img class="sina-emotion" src="' + img.src + '" alt="' + img.alt + '" />';
					$target.insertText(imgText);
				}else{
	...
	var loadEmotions = function(callback) {
		...
		//var emotionsPath = "/image/emotions";
		$.ajax({
			//url:emotionsPath+"/emotions.json",
			url:"http://localhost:8080/City/image/emotions/emotions.json",
			dataType : 'json',
