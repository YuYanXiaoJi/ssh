          ...
          <table class="table table-striped table-hover">
            <thead>
              <tr>
                ...
                <th class="hidden-sm"><span class="glyphicon glyphicon-comment"></span> <span class="visible-lg">回答数</span></th>
                <th><span class="glyphicon  glyphicon glyphicon-user"></span> <span class="visible-lg">关注数</span></th>
            ...
            <tbody>
              <c:forEach items="${questionList}" var="question" >
              	<tr>
              		<td><input type="checkbox" class="input-control" name="checkbox[]" value="${question.id}" /></td>
              		<td class="article-title">${question.title}</td>
	                <td class="hidden-sm">${question.pageView}</td>
	                <td class="hidden-sm">${question.replyCount }</td>
	                <td class="hidden-sm">${question.focusCount }</td>
	                
	                <td>${fn:substring(question.time,0,16)}</td>
	                <td>
	                	<a title="查看问题详情" data-toggle="modal" onclick="getQuestionDetail(${question.id})" >查看详情&nbsp;</a>
	                	<a data-toggle="modal" onclick="getAnswerAll(1,${question.id})" >&nbsp;查看回答</a>
	                </td>
          ...
          <nav>
            <div class="btn-toolbar operation" role="toolbar">
              ...
              <div class="btn-group" role="group">
                <button type="button" id="delete-question" class="btn btn-default" data-toggle="tooltip" data-placement="bottom" title="删除全部选中"  >删除</button>
              </div>
            </div>
            ...
            <ul class="pagination pagenav">
             <c:choose> 
	              <c:when test="${currentPage != 1}">
		              <li>
		              	<a href="<%=basePath%>adminGetQuestionAll.action?currentPage=${currentPage-1}" aria-label="Previous"> <span aria-hidden="true">&laquo;</span> </a>
		              </li>
	              </c:when>
	              <c:otherwise>
	              	   <li class="disabled">
              ...
              <c:if test="${currentPage != 1}">
              	 <li>	
				   <a href="<%=basePath%>adminGetQuestionAll.action?currentPage=1">1</a>
				 </li>     
			  </c:if>
               ...
               <c:if test="${currentPage - 1 >= 2}">
               		<li >
               			<a href="<%=basePath%>adminGetQuestionAll.action?currentPage=${currentPage - 1}">${currentPage - 1}</a>
               		</li>
               </c:if>
              <li class="active">
              	 <a href="<%=basePath%>adminGetQuestionAll.action?currentPage=${currentPage}">${currentPage}</a> 
              </li>
              <c:if test="${currentPage + 1 < totalPage}">	
               ...
               <c:if test="${totalPage != currentPage}">
				  	 <li><a href="<%=basePath%>adminGetQuestionAll.action?currentPage=${totalPage}">${totalPage}</a></li>
			   </c:if>
			    <c:choose> 
	              <c:when test="${currentPage != totalPage }">
		              <li>
		              	<a href="<%=basePath%>adminGetQuestionAll.action?currentPage=${currentPage+1}" aria-label="Next"> <span aria-hidden="true">&raquo;</span> </a>
		              </li>
	              </c:when>
	              <c:otherwise>
	              	   <li class="disabled">
...
<!-- admin 查看 管理  回答 模态框  -->
<div class="modal fade" id="getAnswerAll" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document" style="width:1000px">
		<div class="modal-content">  
            <div class="modal-header">  
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>  
                <h4 class="modal-title" id="myModalLabel">回答列表</h4>  
            </div>  
...
<!-- question的详细信息 -->
<div class="modal fade" id="getQuestionDetail" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document" style="width:1000px">
		<div class="modal-content">  
            <div class="modal-header">  
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>  
            ...
            <div class="modal-body">  
              	<h3 style="text-align: center;" id="modal-question-title"></h3>
              	<div id="modal-question-content" style="text-align: center;" >
              	</div>
            </div>  
            <div class="modal-footer">  
	...
	$('body').on('click','#delete-comment',function(){
		...
		$('#modal-article-content input[type="checkbox"]:checked').each(function(){
			...
			questionComment.push({
				id:$(this).prop('id'),
				flag:Number($(this).prop('value'))
			});
			var args = JSON.stringify(questionComment);
			var url = '<%=basePath%>adminDeleteQuestionAnswer.action';
...
<script type="text/javascript">
	 $('body').on('click', '.modal-footer ul a', function() {
		  ...
		  if($(this).prop('text') == '»') {
		  	 getAnswerAll(modalCurrentPage+1,modalQuestionId);return false;
		  }
		  if($(this).prop('text') == '«') {
		  	getAnswerAll(modalCurrentPage-1,modalQuestionId);return false;
		  }
		  getAnswerAll($(this).prop('text'),modalQuestionId);
	 })
...
<script type="text/javascript">
	...
	var modalQuestionId;
	function getAnswerAll(currentPage,questionId) {
		modalQuestionId = questionId;
		var url = '<%=basePath%>getAnswerAll.action';
		var ar = {'questionId' : Number(questionId), 'modalCurrentPage' : Number(currentPage)};
		var args = JSON.stringify(ar);
		$.post(url,{json:args},function(data){
				var reslut = '';
				for(var i = 0; i < data.answerlist.length; i++) { 
					/* 这个是father（一级回答）  */
					reslut += '<table>'+  /* input的value代表是一级还是二级 0 是一  1 是二 */
								 '<td><input type="checkbox" class="input-control" name="checkbox2[]" id="'+ data.answerlist[i].father.id +'" value="0" /></td>' +
 							     '<td>' + data.answerlist[i].father.content +'</td>' +
 							   '</table>';
 				    for(var j = 0; j < data.answerlist[i].son.length; j++) {
 				    	reslut += '<table style="margin:30px;" > '+  /* input的value代表是一级还是二级 0 是一  1 是二 */
								 '<td><input type="checkbox" class="input-control" name="checkbox2[]" id="'+ data.answerlist[i].son[j].id +'" value="1" /></td>' +
 							     '<td>' +  data.answerlist[i].son[j].content +'</td>' +
 							   '</table>';	
 				    }
 				...
 				if(data.answerlist.length != 0) {
			  	...
			  	$('#modal-article-content').append('<h3>暂无回答</h3>');
			  }
			  $('#getAnswerAll').modal('show'); 
			}
		,'json');
	...
	//清空
	$('#getAnswerAll').on('hidden.bs.modal',function(){
		 $('#modal-article-content').empty();
		 $('.modal .btn-toolbar ul').remove();
	});
...
<script type="text/javascript">
	function getQuestionDetail(questionId) {
		var url = '<%=basePath%>adminGetQuestionDetail.action';
		var ar = {'questionId' : Number(questionId)};
		var args = JSON.stringify(ar);
		$.post(url, {json : args}, function(data) {
				$('#modal-question-title').text(data.question.title);
		 		$('#modal-question-content').html(data.question.content);		
			}
		,'json');
		$('#getQuestionDetail').modal('show');
	}
	...
	$('#delete-question').click(function(){
		 if(window.confirm('此操作不可逆，是否确认？'))
		 {
		 	...
		 	for(var i = 0; i < obj.length; i++) {
		 		questionId.push($(obj[i]).prop('value'));
		 	}
		 	var args = JSON.stringify(questionId);
			 $.ajax({
					type: 'post',
					url: '<%=basePath%>adminDeleteQuestion.action',
					data: {json : args},
					cache: false, //不缓存此页面   
					success: function () {
